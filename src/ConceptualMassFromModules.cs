...
{
    public static class ConceptualMassFromModules
    {
        /// <summary>
        ...
        /// <returns>A ConceptualMassFromModulesOutputs instance containing computed results and the model with any new elements.</returns>
        public static ConceptualMassFromModulesOutputs Execute(Dictionary<string, Model> inputModels, ConceptualMassFromModulesInputs input)
        {
            var output = new ConceptualMassFromModulesOutputs();
            ...
            var masses = new List<ConceptualMass>();

            var unitLayoutDefinition = inputModels["Unit Layout"];

            var conceptualMasses = unitLayoutDefinition.AllElementsOfType<ConceptualMass>().ToList();
            ...
            for (int i = 0; i < conceptualMasses.Count; i++)
            {
                ConceptualMass mass = conceptualMasses[i];

                mass.BottomLevel = new LevelReference(levels.First(l => l.Id == mass.LevelIds.First()));
                mass.TopLevel = new LevelReference(levels.First(l => l.Id == mass.LevelIds.Last()));

                if (mass.TopLevel.Elevation == mass.BottomLevel.Elevation)
                {
                    mass.TopLevel = new LevelReference(levels.First(l => l.Id == conceptualMasses[i + 1].LevelIds.First()));
                }

                var representation = new Elements.Geometry.Solids.Extrude(mass.Profile, mass.Height - 0.001, Vector3.ZAxis, false);
                mass.Representation = representation;
